...
soft_score_root: str = config.get("DISSECTSCORE", "soft_score_root")                                   
hard_score_root: str = config.get("DISSECTSCORE", "hard_score_root")               
floc_score_root: str = config.get("DISSECTSCORE", "floc_score_root")                                           
food_floc_score_root: str = config.get("DISSECTSCORE", "food_floc_score_root")                      

person_number: int = 8                                                                                                                      
floc_number_for_each_dissect_label: int = 3                                                                                                 
root_end: str = '.npy'                                                                                                                      
dissect_labels: list = ['faces', 'bodies', 'places', 'words', 'food']                                                                       
new_dissect_labels: list = ['FFA', 'EBA', 'RSC', 'VWFA', 'FOOD']
floc_labels: list = ['faces', 'adult', 'child', 'bodies', 'body', 'limb', 'places', 'house', 'corridor', 'word', 'number', 'characters']
...
food_floc_score_root_list: list = [food_floc_score_root.format(i) for i in range(1, person_number + 1)]
...
def get_coherrence(person_index: int, dissect_index: int): 
    ...
    if dissect_index < len(dissect_labels) - 1:
        ...
        floc_data = np.load(food_floc_score_root_list[person_index] + root_end)
        soft_coherrence_list.append(np.corrcoef(soft_data, floc_data)[0, 1])
        hard_coherrence_list.append(np.corrcoef(hard_data, floc_data)[0, 1])
