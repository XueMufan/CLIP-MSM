...
def load_model_performance(model, output_root=".", subj=1, measure="corr", roi="", n_session=34):
    if measure == "pvalue":
        measure = "corr"
        pvalue = True
    else:
    ...
    if type(model) == list:
        ...
        out = np.load(
            "%s/output/encoding_results/%s_%s/subj%d/%d_session/%s_%s_%s.p"
            % (output_root, model, roi_tag, subj, n_session, measure, model, roi_tag),
            allow_pickle=True,
        )
