...
if __name__ == "__main__":
    ...
    if args.compute_ev:
        ...
        print("tag:", tag)
        print("roi:", roi)
        print("save_dir:", "%s/evs_subj%02d%s.npy" % (output_dir, args.subj, tag))
        all_evs = compute_ev(args.subj, roi, args.biascorr, args.zscored_input)
        np.save("%s/evs_subj%02d%s.npy" % (output_dir, args.subj, tag), all_evs)

        plt.figure()
        plt.hist(all_evs)
        plt.title("Explainable Variance across Voxels (subj%02d%s)" % (args.subj, tag))
        plt.savefig("%s/evs_subj%02d%s.png" % (figure_dir, args.subj, tag))
        ...
        if roi is not None:
            roi_mask = np.load(
                ...
                % (output_dir, args.subj, args.subj, roi)
            )
