...
def ridge_cv(
    ...
    return (
        corrs,  # 所有体素预测结果和真实结果的相关性
        rsqs,   # 所有体素预测结果和真实结果的R²
        clf.mean_cv_scores.cpu().numpy(),   # 平均cross validation得分
        clf.best_l_scores.cpu().numpy(),    # 最佳cross validation得分
        clf.best_l_idxs.cpu().numpy(),
        [yhat, y_test],                     # 【预测值、真实值】
        weights.cpu().numpy(),              # 权重
        bias.cpu().numpy(),                 # 偏置
        clf,
    )
...
def bootstrap_test(
    ...
    try:
        weights = np.load(
            "%s/encoding_results/subj%d/weights_%s_%s.npy"
            % (saving_dir, subj, model_name,roi)
        )
        bias = np.load(
            "%s/encoding_results/subj%d/bias_%s_%s.npy"
            % (saving_dir, subj, model_name,roi)
        )
    ...
    np.save("%s/rsq_dist_%s_%s.npy" % (outpath, model_name,roi), rsq_dists)
