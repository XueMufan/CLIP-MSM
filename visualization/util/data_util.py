...
def load_model_performance(model, roi, output_root=".", subj=1, measure="corr"):
    if measure == "pvalue":
        measure = "corr"
        pvalue = True
    else:
    ...
    if type(model) == list:
        ...
        out = np.load(
            "%s/output/encoding_results/%s_%s/subj%d/34_session/%s_%s_%s.p"
            % (output_root, model, roi, subj, measure, model, roi),
            allow_pickle=True,
        )
