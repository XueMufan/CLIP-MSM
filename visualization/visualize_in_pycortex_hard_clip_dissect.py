...
def load_fdr_mask(OUTPUT_ROOT, roi, model, fdr_mask_name, subj):
    ...
    try:
        sig_mask = np.load(
            "%s/output/ci_threshold/%s_%s_34session_fdr_p_subj%01d.npy" % (OUTPUT_ROOT, model, roi, subj)
        )[0].astype(bool)
        return sig_mask
...
def make_volume(
    ...
    if vals is None:
        if (
            ...
            vals = load_model_performance(
                model, roi=roi, output_root=OUTPUT_ROOT, subj=subj, measure=measure
            )
        print("model:" + model)
    ...
    if mask_with_significance:
        if sig_method == "fdr":
            sig_mask = load_fdr_mask(OUTPUT_ROOT, roi, model, fdr_mask_name, subj)
            if sig_mask is None:
    ...
    labels = ['faces', 'bodies', 'places', 'words', 'food']
    val = np.full((sum(cortical_mask.flatten())),np.nan)
    # val = np.zeros((sum(cortical_mask.flatten())),dtype=np.float64)
    roi = "SELECTIVE_ROI"
    roi_in_nsdgeneral_mask = np.load("%s/output/ci_threshold/%s_in_nsdgeneral_mask_subj%02d.npy"%(OUTPUT_ROOT,roi,subj))
    roi_to_nsdgeneral_index = np.load("%s/output/ci_threshold/%s_to_nsdgeneral_index_subj%02d.npy"%(OUTPUT_ROOT,roi,subj))

    val[roi_in_nsdgeneral_mask] = 0
    for i, label in enumerate(labels):
        df = pd.read_csv('%s/hard-clip-dissect_hy_free_%s_index.csv'%(loading_path,label))
        data_array = df.to_numpy()
        data_array = data_array.reshape(len(data_array))
        a = np.zeros(len(roi_to_nsdgeneral_index),dtype=int)
        a[data_array] = i + 1
        # print(a.dtype())
        non_neg_one_mask = (a != 0)
        print(np.count_nonzero(non_neg_one_mask))
        # print(np.count_nonzero(~np.isnan(a)))
        val[roi_in_nsdgeneral_mask] += a[roi_to_nsdgeneral_index]
        print(np.count_nonzero(~np.isnan(val)))
        non_neg_one_mask = (val != 0) & (~np.isnan(val))
        print(np.count_nonzero(non_neg_one_mask))
