...
def make_volume(
    ...
    floc_value = load_mask_from_nii(loading_path)

    nsdgeneral_roi = load_mask_from_nii(PPdataPath + '/subj0%d/func1pt8mm/roi/nsdgeneral.nii.gz'%subj)
    nsdgeneral_mask = nsdgeneral_roi != -1
    floc = floc_value[nsdgeneral_mask]
    min_val = np.min(floc)
    max_val = np.max(floc)

    # 对数组进行归一化处理
    normalized_floc = (floc - min_val) / (max_val - min_val)
    val[roi_in_nsdgeneral_mask] = normalized_floc[roi_in_nsdgeneral_mask]
    vmin=0
    vmax=1
