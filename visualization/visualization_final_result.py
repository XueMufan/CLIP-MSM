...
def fdr_correct_p(var):
    ...
    return fdr_p
from visualization.visualize_in_pycortex_2 import make_volume
import cortex
...
for label in labels:
    for i in range(1,9):
        ...
        for k in volumes.keys():
                file_path = "%s/clip2brain/SELECTIVE_ROI_training_figures"%(ROOT)
                if not os.path.exists(file_path):
                    os.makedirs(file_path)
                filename = "%s/clip2brain/SELECTIVE_ROI_training_figures/%s_%s_34session_subj0%d"%(ROOT, model, roi, i)
