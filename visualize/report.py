...
def generate_html_summary( layer, output_folder,top_name=None, features=None, thresholds=None,
        imsize=None, imscale=72, tally_result=None,
        gridwidth=None, gap=3, limit=None, force=False, verbose=False):
    ed = expdir.ExperimentDirectory(output_folder)
...
#     top = np.argsort(maxfeature, 0)[:-1 - settings.TOPN:-1, :].transpose()
    ...
    html.append('%d/%d units covering %d concepts with score &ge; %.2f' % (
...
#         rendered_order = rendered_order[:limit]
    ...
    for label_order, record in tqdm(enumerate(rendered_order)):
        unit = int(record['unit'].split(" ")[1]) # zero-based unit indexing
        ...
        if force or not ed.has('html/%s' % imfn):
            ...
            for x, index in enumerate(top_name[unit]):
                row = x // gridwidth
                col = x % gridwidth
                image = imageio.imread(index,pilmode='RGB')
#                 image=np.repeat()
...
#                     print("imsize: ",imsize)
                    ...
                    vis = np.array(Image.fromarray(np.uint8(vis*255)).resize((imsize, imsize)))
        ...
        html.append('<div class="info">' +
            ...
            '<span class="iou">score %.2f</span>' % float(record['score']) +
            '</div>')
        html.append(
